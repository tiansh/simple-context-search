...
const searchTextWithAccessKey = /R/i.test(searchTextLocalized) ? searchTextLocalized.replace(/R/i, '&$&') : searchTextLocalized + ' (&R)';
...
const assignAccessKeys = function (searchList) {
  ...
  searchList.forEach(search => {
    let assigned = false;
    search.displayName = search.name.replace(/&/g, '&&');
    search.displayName = search.displayName.replace(/[a-z0-9]/ig, character => {
      if (assigned) return character;
      if (keyUsed.has(character.toUpperCase())) return character;
      assigned = true;
      keyUsed.add(character.toUpperCase());
      return '&' + character;
    });
    if (!assigned) pending.push(search);
  });
  let index = 0;
  pending.forEach(search => {
    while (index < 36) {
      const n = index++;
      const accessKey = n < 10 ? '' + (n + 1) % 10 : n.toString(36).toUpperCase();
      if (!keyUsed.has(accessKey)) {
        search.displayName += ` (&${accessKey})`;
        return;
      }
    }
  });
